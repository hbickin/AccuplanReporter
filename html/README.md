# Kesimhane Asorti Raporu — HTML + jQuery sürümü

Accuplan iş emirlerinin (`[Accuplan].[dbo].[WorkOrder]`) `document` alanındaki XML'i okuyup
**kesimhane asorti raporunu** tarayıcıda üreten ve Excel'e aktaran uygulama.

Buradaki komutlar bu klasörden (`html/`) çalıştırılır.
...
deponun **[`../rdl`](../rdl)** klasöründe (ya da `rdl` dalında).
